...
//var BaseURL = 'https://kwapi-api-iobiovqpvk.cn-beijing.fcapp.run'
 var BaseURL = 'http://10.168.1.107:9000'
  ...
  retryRequest(`${baseUrl}/mp3?rid=${rid}`)
    .then(data => {
      ...
      if (data.includes('版权')) {
        ...
        playMusicOnServer(mp3Url);/////////////// 将音乐链接传递给后端////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////获取到了音乐链接后 用VLC播放
function playMusicOnServer(mp3Url) {
  // 发送请求到后端
  fetch(BaseURL+'/play-music', {
    method: 'POST',
    headers: {
  ...
  .then(data => {
    console.log(data);
    // 成功播放音乐后，后端返回结果，切换播放按钮图标为暂停
     const divElement = document.querySelector('.plyr.plyr--full-ui.plyr--audio.plyr--html5.plyr--stopped');
     const playButton = document.querySelector('.plyr__controls__item.plyr__control[data-plyr="play"]');
     divElement.classList.remove('plyr--paused');
     divElement.classList.add('plyr--playing');
     playButton.classList.add('play-pause', 'plyr__control--pressed');
...
function handlePlayButtonClick() {
  // 获取plyr播放按钮的aria-label值
  const ariaLabel = playButton.getAttribute('aria-label');
  // 根据aria-label值更新沉浸状态的暂停、播放图标
  if (ariaLabel === 'Pause') {
  ...
  // 切换类和属性
  playButton.classList.toggle("play-pause");
  playButton.classList.toggle("plyr__control--pressed");
  playButton.setAttribute("aria-label", ariaLabel === 'Pause' ? 'Play' : 'Pause');

  // 切换div.plyr类
  divElement.classList.toggle("plyr--paused");
  divElement.classList.toggle("plyr--playing");
}
